...
import { Button, Modal, Input, Table, Tag, Space } from "antd";
    ...
    if (props.faqs) {
      ...
      props.faqs.length > 0 &&
        props.faqs.map((el) => {
          ...
          } else if (requiredCheck(el["eng"]) && el["eng"].answer) {
            el.language = "Англи хэл";
          } else if (requiredCheck(el["mn"]) && el["mn"].answer) {
            el.language += "Монгол хэл";
          }

          if (el[cookies.language]) {
            const { answer, question } = el[cookies.language];
            el.answer = answer;
            el.question = question;
          } else {
            if (el["mn"] && el["mn"].answer) {
              el.answer = el["mn"].answer;
              el.question = el["mn"].question;
            } else if (el["eng"] && el["eng"].answer) {
              el.answer = el["eng"].answer;
              el.question = el["eng"].question;
            }
          }
