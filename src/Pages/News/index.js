...
import { Button, Modal, Input, Select, Table, Tag, Space } from "antd";
...
import { requiredCheck } from "src/lib/inputRegex";
...
const News = (props) => {
  ...
  const [columns, setColumns] = useState([
    ...
    {
      dataIndex: "cloneCategories",
      key: "cloneCategories",
      title: "Ангилал",
      status: true,
      ...getColumnSearchProps("categories"),
      render: (text, record) => {
        return record.cloneCategories.map((el) => (
    ...
    if (props.allNews) {
      ...
      props.allNews.length > 0 &&
        props.allNews.map((el) => {
          ...
          el.language = "";
          if (requiredCheck(el["mn"]) && requiredCheck(el["eng"])) {
            el.language = "Англи хэл, Монгол хэл";
          } else if (requiredCheck(el["eng"]) && el["eng"].name) {
            el.language = "Англи хэл";
          } else if (requiredCheck(el["mn"]) && el["mn"].name) {
            el.language += "Монгол хэл";
          }

          if (el[cookies.language]) {
            const { name } = el[cookies.language];
            el.name = name;
          } else {
            if (el["mn"] && el["mn"].name) {
              el.name = el["mn"].name;
          ...
          el.star = el.star == true ? "Онцлох" : "Энгийн";
          ...
          el.updateAt = moment(el.updateAt)
            .utcOffset("+0800")
            .format("YYYY-MM-DD HH:mm:ss");
          let cloneCategories = el.categories;
          cloneCategories = cloneCategories.map((el) => {
            let name = "---";
            if (el[cookies.language] && el[cookies.language].name) {
              name = el[cookies.language].name;
            } else if (cookies.language === "mn") {
              name = el["eng"].name + " / Монгол орчуулга оруулаагүй /";
            } else if (cookies.language === "eng") {
              name = el["mn"].name + " / Англи орчуулга оруулаагүй /";
            }

            return name;
          });

          el.cloneCategories = cloneCategories;
  ...
  const queryBuild = () => {
    ...
    // if (querys)
    //   query += `select=${
    //     querys && querys.select && querys.select[0].join(" ").replace(",", " ")
    //   }`;
    return query;
  };
  ...
  const exportExcel = async () => {
    const query = queryBuild();
    let response = await axios.get("news/excel?" + query);
    ...
    if (response) {
      ...
      excelData = data.map((el) => {
        cloneColumns.map((col) => {
          ...
          if (col.key === "name" && col.status === true) {
            let lang = "";
            if (el[cookies.language] && el[cookies.language].name) {
              lang = el[cookies.language].name;
            } else if (el["mn"] && el["mn"].name) {
              lang = el["mn"].name;
            } else if (el["eng"] && el["eng"].name) {
              lang = el["eng"].name;
