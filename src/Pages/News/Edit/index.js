...
const Edit = (props) => {
  ...
  useEffect(() => {
    if (props.news) {
      if (props.news[cookies.language] && props.news[cookies.language].name) {
        const { name, details } = props.news[cookies.language];
        props.news.name = name;
        props.news.details = details;
        delete props.news["eng"];
        delete props.news["mn"];
      }
