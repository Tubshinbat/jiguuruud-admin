...
const Add = (props) => {
    ...
    if (categories) {
      categories.map((el) => {
        ...
        if (el[cookies.language] && el[cookies.language].name) {
          title = el[cookies.language].name;
        } else {
          if (cookies.language === "mn") {
            title = el["eng"].name + " / Монгол орчуулга ороогүй /";
          } else {
            title = el["mn"].name + " / Англи орчуулга ороогүй /";
          }
        }
  ...
  useEffect(() => {
    if (props.page) {
      if (props.page[cookies.language] && props.page[cookies.language].name) {
        const { name, pageInfo } = props.page[cookies.language];
        props.page.name = name;
        props.page.pageInfo = pageInfo;
        delete props.page["eng"];
        delete props.page["mn"];
      }
